function Invoke-WinEnum{
    ...
    if($SysInfo){
        ...
        if($Lastupdate){
           Write-Host $Lastupdate.InstalledOn
        }
        else{
            "Unkown`n" 
        }    
    ...
    if($NetInfo){
        ...
        "---------------------------"
        "Netstate Established connections and processes"
        "---------------------------`n"
        ...
        netstat -ano | Select-String -Pattern '\s+(TCP|UDP)' | ForEach-Object {
            ...
            {            
                ...
                if($netstat.State -eq 'ESTABLISHED' ){
                    $netstat | Format-List ProcessName,LocalAddress,LocalPort,RemoteAddress,RemotePort,State
                }
            } 
        } 
